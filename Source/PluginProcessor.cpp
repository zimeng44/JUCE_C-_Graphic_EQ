...
void GraphicEQAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    std::vector<int> freqs = {32, 63, 125, 250, 500, 1000, 2000, 4000, 8000, 16000};
    
    for (int bandNum = 0; bandNum < 10; bandNum++) {
        eqBands[bandNum].prepare(freqs[bandNum], sampleRate, *parameters.getRawParameterValue(juce::String(freqs[bandNum]) + "gainValue"));
        eqBands[bandNum].initializeVTS(parameters);
    }
...
//    std::vector<std::unique_ptr<juce::RangedAudioParameter>> params;
    ...
    for (int i = 0; i < 10; i++){
        juce::ParameterID pid = juce::ParameterID(juce::String(freqs[i]) + "gainValue", 1);
        
        layout.add(std::make_unique<juce::AudioParameterFloat> (pid,
                                                                juce::String(freqs[i]) + "GainValue",
                                                                juce::NormalisableRange<float>(0.01, 2.0, 0.01),
                                                                1.0));
    }
